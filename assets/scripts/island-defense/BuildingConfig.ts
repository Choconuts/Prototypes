...
@ccclass('BuildingConfig')
export class BuildingConfig extends Component {
    @property
    unitKey: string = 'house-unit'
    @property(ConfigView)
    ...
    async tryWork(): Promise<boolean> {
        const creature = await GameMap.instance.generateCreature(this.workingInfo, this.unitKey, false, true);
